...
from music_module import play_music_thread2
import threading

music_queue = Queue()

music_play_thread = threading.Thread(target=play_music_thread2, args=(music_queue,))
...
def plot(pose_result, plot_size_redio, show_points=None, show_skeleton=None):
    class _Annotator(Annotator):
        ...
        def kpts(self, kpts, shape=(1280, 720), radius=5, line_thickness=2, kpt_line=True):
...
def parse_args():
    ...
    parser.add_argument('--save_dir', default='C:/Users/user/Downloads/Exercise-Counter-with-YOLOv8-on-NVIDIA-Jetson-main\Exercise-Counter-with-YOLOv8-on-NVIDIA-Jetson-main', type=str, help='path to save output')
...
def open_webcam():
    ...
    while cap is None or not cap.isOpened():
        # Close the previous webcam if it's open
        if cap is not None:
            cap.release()

        # Ask the user to enter the webcam index (typically 0 for built-in webcam)
        #webcam_index = input("Enter the webcam index (e.g., 0): ")
...
def main():
    ...
    model=YOLO('C:/Users/user/Downloads/Exercise-Counter-with-YOLOv8-on-NVIDIA-Jetson-main/Exercise-Counter-with-YOLOv8-on-NVIDIA-Jetson-main/yolov8n-pose_openvino_model/')
            ...
            if reaching != reaching_last:
                ...
                if not reaching and state_keep:
                    ...
                    if counter==10:
                        ...
                        #music_queue.put(True)
                        # music_path='D:/AI_exam/YOLOV_team_project/downloaded_audio/『Lyrics AMV』Boku no Kokoro no Yabai Yatsu【Shayou - Yorushika】Opening Full.mp3'
